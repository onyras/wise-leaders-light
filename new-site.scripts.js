...
// Fit Quiz
const fitQuiz = document.querySelector('.fit-quiz__card');
if (fitQuiz) {
...
// ==========================================
// Fit Quiz Drawer (slide-in)
// ==========================================
const fitQuizLauncher = document.getElementById('fitQuizLauncher');
const fitDrawer = document.getElementById('fitDrawer');
if (fitQuizLauncher && fitDrawer) {
    const drawerVideo = document.getElementById('fitDrawerVideo');
    const closeTargets = fitDrawer.querySelectorAll('[data-close]');

    const openDrawer = () => {
        fitDrawer.setAttribute('aria-hidden', 'false');
        document.body.classList.add('fit-drawer-open');
        if (drawerVideo) {
            try {
                drawerVideo.currentTime = 0;
                drawerVideo.muted = true;
                const playPromise = drawerVideo.play();
    ...
    const closeDrawer = () => {
        fitDrawer.setAttribute('aria-hidden', 'true');
        document.body.classList.remove('fit-drawer-open');
        if (drawerVideo) {
            drawerVideo.pause();
        }
    };

    fitQuizLauncher.addEventListener('click', openDrawer);
    closeTargets.forEach(el => el.addEventListener('click', closeDrawer));
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && fitDrawer.getAttribute('aria-hidden') === 'false') {
            closeDrawer();
        }
    });
}
